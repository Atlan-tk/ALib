...
- `AList(T)::rm_p(self, p)` / `AList(T)::take_p(self, p, tar)` 都会按元素地址直接操作节点，性能较好，但使用上也更危险。
- 调用者必须确认 `p` 来自这个链表当前仍然挂接的节点；最稳妥的来源是同一个 `list` 的 `at()` 返回值或迭代器当前元素。
- 一旦该元素已经被删除、链表已经析构/拷贝重建，或者 `p` 实际来自别的链表，就不要再传给 `rm_p` / `take_p`。
- `take_p` 在 `tar != NULL` 时会把元素内容取出并解除挂接；对于带资源的对象类型，后续资源生命周期转交给 `tar`，调用者需要按正常对象去析构它。
- 如果拿不准元素是否仍挂在链表上，优先使用 `rm(index)` / `take(index, ...)`。
...
- 每一种业务信号都继承自 `ASignal`
- 运行时先用 `a_signal_alloc()` 申请一个全局信号 `id`
- 接收者通过 `a_signal_connection(id, addressee, target)` 绑定回调
- 发送者构造信号对象后调用 `a_signal_transmit(...)`
- 当前源码只提供接收者辅助基类 `AReceEnd`；发送端若要长期保存 `id` 或做自动清理，需要自行封装
...
- `id` 必须由 `a_signal_alloc()` 分配，未分配或越界的 `id` 会设置 `AEXC_outdomain`
- 同一个 `id` 下，同一个 `addressee` 只能绑定一个 `target`
- 重复注册会设置 `AEXC_repeat_write`
- `a_signal_disconnect(id, addressee)` / `a_target_disconnect(addressee, id)` 解绑单条连接
- `a_signal_disconnect_all(id)` / `a_target_disconnect_all(addressee)` 解绑整组连接
- 已分配但当前没有监听者的 `id` 上，发送与批量断开都是 no-op
- 回调接收到的是只读 `const ASignal*`，接收者不应修改信号内容
- `signal->sender` 只是业务透传指针；lib 不管理它指向对象的生命周期
- 发射前会复制当前接收者列表，并按这份快照顺序调用；因此回调里新增 `connection` 只影响下一次派发

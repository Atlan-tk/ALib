...
- `AList(T)::rm_p(self, p)` 和 `AList(T)::take_p(self, p, tar)` 都会按元素地址直接定位并操作节点，适合你已经持有链表内元素指针的场景。
- 这两个接口相对危险：调用者必须确认 `p` 来自当前这个 `list`，并且对应元素此刻仍然挂在链表上。
- 推荐只把同一个链表的 `at()` 返回值或迭代器当前元素传给 `rm_p` / `take_p`；元素一旦被删除，或者链表已经析构/拷贝重建，旧指针就不应继续使用。
- `take_p` 在 `tar != NULL` 时会把元素内容取出并解除挂接；如果元素类型本身持有堆内存、文件句柄等资源，资源所有权会转移到 `tar`，后续应由调用者负责正常析构。
- 如果不能明确保证“该元素仍挂在这条链表上”，请改用 `rm(index)` / `take(index, ...)`。

...
__noused __weak void A_OBJ_INIT(My_class)(My_class* self){
    //构造函数中无需检查self是否为null
    printf("hello, alib\n");
    self->s = "alib sample with type system";
}

__noused __weak void A_OBJ_COPY(My_class)(My_class* self, const My_class* that){
    printf("hello, alib\n");
    self->s = that->s;
}

__noused __weak void A_OBJ_DEST(My_class)(My_class* self){
    self->s = nullptr;
    printf("bay, alib\n");
...
__noused __weak int A_OBJ_CMPD(My_class)(const My_class* self, const My_class* that){
...
//覆盖父类函数
__noused __weak void A_SET_VTAB(My_class_sub)(My_class_sub* self){
    A_COVER_FUNC(self, My_class, print, My_class_sub_print);
}

__noused __weak void A_OBJ_INIT(My_class_sub)(__noused My_class_sub* self){
    printf("hello, alib, sub class\n");
}

__noused __weak void A_OBJ_COPY(My_class_sub)(__noused My_class_sub* self, __noused const My_class_sub* that){
    printf("hello, alib, sub class\n");
}

__noused __weak void A_OBJ_DEST(My_class_sub)(__noused My_class_sub* self){

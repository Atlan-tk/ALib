...
__unused __weak void A_OBJ_INIT(My_class)(My_class* self){
    //构造函数中无需检查self是否为null
    printf("hello, alib\n");
    self->s = "alib sample with type system";
}

__unused __weak void A_OBJ_COPY(My_class)(My_class* self, const My_class* that){
    printf("hello, alib\n");
    self->s = that->s;
}

__unused __weak void A_OBJ_DEST(My_class)(My_class* self){
    self->s = nullptr;
    printf("bay, alib\n");
...
__unused __weak int A_OBJ_CMPD(My_class)(const My_class* self, const My_class* that){
...
//覆盖父类函数
__unused __weak void A_SET_VTAB(My_class_sub)(My_class_sub* self){
    A_COVER_FUNC(self, My_class, print, My_class_sub_print);
}

__unused __weak void A_OBJ_INIT(My_class_sub)(__unused My_class_sub* self){
    printf("hello, alib, sub class\n");
}

__unused __weak void A_OBJ_COPY(My_class_sub)(__unused My_class_sub* self, __unused const My_class_sub* that){
    printf("hello, alib, sub class\n");
}

__unused __weak void A_OBJ_DEST(My_class_sub)(__unused My_class_sub* self){

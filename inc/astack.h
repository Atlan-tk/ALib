...
#define AStack_Generate(T)                                                          \
    ...
    static inline void __Ascf(T,iter_prev)(AIter(AStack(T))* it){                   \
        it->i--; it->p = __Ascf(T,at)(it->con, it->i);                              \
                                                                                    ...
                                                                                    \
    __noused static inline void A_OBJ_DEST(AStack(T))                               \
    (AStack(T)* self){                                                              \
    ...
    __noused static inline void A_OBJ_INIT(AStack(T))                               \
    (AStack(T)* self){                                                              \
        __Adeq_init(&self->deq, sizeof(T));                                         \
        self->f = &A_FUNC_TAB(AStack(T));                                           \
    }                                                                               \
    __noused static inline void A_OBJ_COPY(AStack(T))                               \
    (AStack(T)* self, const AStack(T)* that){                                       \
        self->f = that->f;                                                          \
        __Adeq_init(&self->deq, sizeof(T));                                         \
        for(uint32_t i = 0; i < that->deq.num; i++){                                \
            aTry(__Ascf(T,pushBack)(self, *__Ascf(T,at)(that, i));)aExc{            \
                return;                                                             \
            }                                                                       \
        }                                                                           \
    }                                                                               \
    __noused static inline int A_OBJ_CMPD(AStack(T))                                \

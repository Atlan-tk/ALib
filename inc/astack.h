...
#define AStack_Generate(T)                                                          \
    ...
    static inline void __Ascf(T,iter_prev)(AIter(AStack(T))* it){                   \
        it->i--; it->p = __Ascf(T,at)(it->con, it->i);                              \
                                                                                    ...
                                                                                    \
    __unused __weak void A_OBJ_DEST(AStack(T))(AStack(T)* self){                    \
        for(uint32_t i = 0; i < self->deq.num; i++){                                \
            A_DEST(T, *(T*)__Ascf(T,at)(self, i));                                  \
        }                                                                           \
        __Adeq_dest(&self->deq);                                                    \
    }                                                                               \
    __unused __weak void A_OBJ_INIT(AStack(T))(AStack(T)* self){                    \
        __Adeq_init(&self->deq, sizeof(T));                                         \
        self->f = &A_FUNC_TAB(AStack(T));                                           \
    }                                                                               \
    __unused __weak void A_OBJ_COPY(AStack(T))(AStack(T)* self, const AStack(T)* that){     \
        self->f = that->f;                                                          \
        __Adeq_init(&self->deq, sizeof(T));                                         \
    ...
    __unused __weak int A_OBJ_CMPD(AStack(T))(const AStack(T)*self,const AStack(T)*that){   \
        int ret = 0;                                                                \

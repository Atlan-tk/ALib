...
__noused __weak void A_OBJ_INIT(AFile)(AFile* self){
    self->name = A_INIT(AText);
}
__noused __weak void A_OBJ_DEST(AFile)(AFile* self){
    if(self->fp != nullptr) fclose(self->fp);
    A_DEST(AText, self->name);
}
__noused __weak void A_OBJ_COPY(AFile)(AFile* self, const AFile* that){
    self->name = A_COPY(AText, that->name);
    if(AText_empty(&self->name)) self->name = AText_new("(null)");
}
__noused __weak int A_OBJ_CMPD(AFile)(const AFile* self, const AFile* that){
...
__noused __weak void A_OBJ_COPY(ARFile)(ARFile* self, const ARFile* that){
    self->mem = that->mem;
    if(self->mem == nullptr){
...
__noused __weak void A_OBJ_COPY(AWFile)(AWFile* self, __noused const AWFile* that){
...
__noused __weak void A_OBJ_COPY(APFile)(APFile* self, __noused const APFile* that){

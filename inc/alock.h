...
static void A_OBJ_INIT(AMtx)(AMtx* self){
    if(mtx_init(&((ALock*)self)->mtx, mtx_plain) != thrd_success) {
        aExcSet(AEXC_system_error);
    }
}
static void A_OBJ_DEST(AMtx)(AMtx* self){
    mtx_destroy(&((ALock*)self)->mtx);
}
static void A_OBJ_COPY(AMtx)(AMtx* self, __unused const AMtx* that){
    if(mtx_init(&((ALock*)self)->mtx, mtx_plain) != thrd_success) {
        aExcSet(AEXC_system_error);
    }
}
...
static void A_OBJ_INIT(ARecursion)(ARecursion* self){
    if(mtx_init(&((ALock*)self)->mtx, mtx_plain | mtx_recursive) != thrd_success) {
        aExcSet(AEXC_system_error);
    }
}
static void A_OBJ_DEST(ARecursion)(ARecursion* self){
    mtx_destroy(&((ALock*)self)->mtx);
}
static void A_OBJ_COPY(ARecursion)(ARecursion* self, __unused const ARecursion* that){
    if(mtx_init(&((ALock*)self)->mtx, mtx_plain | mtx_recursive) != thrd_success) {
        aExcSet(AEXC_system_error);
    }
}
...
static inline void __AMtxRW_reset(AMtxRW* self){
    self->reader_num = 0;
    self->writer_wait = 0;
...
static void A_OBJ_INIT(AMtxRW)(AMtxRW* self){
    __AMtxRW_reset(self);
...
static void A_OBJ_DEST(AMtxRW)(AMtxRW* self){
...
static void A_OBJ_COPY(AMtxRW)(AMtxRW* self, __unused const AMtxRW* that){

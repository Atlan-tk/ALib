...
#define ADeque_Generate(T)                                                          \
    ...
    static inline void __Adqf(T,iter_prev)(AIter(ADeque(T))* it){                   \
        it->i--; it->p = __Adqf(T,at)(it->con, it->i);                              \
                                                                                    ...
                                                                                    \
    __unused static void A_OBJ_DEST(ADeque(T))(ADeque(T)* self){                    \
        for(uint32_t i = 0; i < self->deq.num; i++){                                \
            A_DEST(T, *(T*)__Adqf(T,at)(self, i));                                  \
        }                                                                           \
        __Adeq_dest(&self->deq);                                                    \
    }                                                                               \
    __unused static void A_OBJ_INIT(ADeque(T))(ADeque(T)* self){                    \
        __Adeq_init(&self->deq, sizeof(T));                                         \
        self->f = &A_FUNC_TAB(ADeque(T));                                           \
    }                                                                               \
    __unused static void A_OBJ_COPY(ADeque(T))(ADeque(T)* self, const ADeque(T)* that){     \
        self->f = that->f;                                                          \
        __Adeq_init(&self->deq, sizeof(T));                                         \
    ...
    __unused static int A_OBJ_CMPD(ADeque(T))(const ADeque(T)*self,const ADeque(T)*that){   \
        int ret = 0;                                                                \

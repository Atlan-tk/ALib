...
#define AList_Generate(T)                                                                   \
    ...
    static inline void __Alsf(T,iter_prev)(AIter(AList(T))* it){                            \
        ...
        it->i--;                                                                            \
                                                                                            ...
                                                                                            \
    __noused __weak __visibility(protected) void A_OBJ_DEST(AList(T))(AList(T)* self){      \
        __Alist_dest(&self->list);                                                          \
    }                                                                                       \
    __noused __weak __visibility(protected) void A_OBJ_INIT(AList(T))(AList(T)* self){      \
    ...
    __noused __weak __visibility(protected) void  A_OBJ_COPY(AList(T))                      \
    (AList(T)* self, const AList(T)* that){                                                 \
        self->f = that->f;                                                                  \
        __Alist_copy(&self->list, &that->list);                                             \
    }                                                                                       \
    __noused __weak __visibility(protected) int  A_OBJ_CMPD(AList(T))                       \

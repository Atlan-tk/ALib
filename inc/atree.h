...
#define ATree_Generate(TK,TV)                                                                   \
    ...
    static inline TK __Atrf(TK,TV,iter_getk)(AIter(ATree(TK,TV)) it){                           \
        ...
        return container_of(it.p,__Atr_data(TK,TV),v)->k;                                       \
                                                                                                ...
                                                                                                \
    __noused __weak void A_OBJ_INIT(ATree(TK,TV))(ATree(TK,TV)* self){                          \
    ...
    __noused __weak void A_OBJ_DEST(ATree(TK,TV))(ATree(TK,TV)* self){                          \
        __Atree_dest(&self->tree);                                                              \
    }                                                                                           \
    __noused __weak void A_OBJ_COPY(ATree(TK,TV))(ATree(TK,TV)* self, const ATree(TK,TV)* that){\
        self->f = that->f;                                                                      \
        __Atree_copy(&self->tree, &that->tree);                                                 \
    }                                                                                           \
    __noused __weak int  A_OBJ_CMPD(ATree(TK,TV))(const ATree(TK,TV)*self,const ATree(TK,TV)*that){ \

...
#define ALine_Generate(T)                                                               \
    ...
    static inline void __Alnf(T, iter_prev)(AIter(ALine(T))* it){                       \
        it->p--, it->i--;                                                               \
                                                                                        ...
                                                                                        \
    __noused static inline void A_OBJ_DEST(ALine(T))(ALine(T)* self){                   \
        for(uint32_t i = 0; i < self->arr.num; i++){                                    \
            A_DEST(T, self->p[i]);                                                      \
        }                                                                               \
        __Aarr_dest(&self->arr);                                                        \
    }                                                                                   \
    __noused static inline void A_OBJ_INIT(ALine(T))(ALine(T)* self){                   \
        self->arr.size = sizeof(T); self->f = &A_FUNC_TAB(ALine(T));                    \
    }                                                                                   \
    __noused static inline void A_OBJ_COPY(ALine(T))                                    \
    ...
    __noused static inline int  A_OBJ_CMPD(ALine(T))                                    \

...
AStack_Define(AString);
AStack_Generate(AString);
A_TYPE_REGISTER(AStack(AString));
...
static void test_astack_astring(void) {
    RAII(AStack(AString)) st = A_INIT(AStack(AString));
    assert(st.f->empty(&st));
    ...
    RAII(AStack(AString)) cp = A_COPY(AStack(AString), st);
    AString *top_copy = cp.f->at(&cp, 1);
    top_copy->f->pushBack(top_copy, '!');
